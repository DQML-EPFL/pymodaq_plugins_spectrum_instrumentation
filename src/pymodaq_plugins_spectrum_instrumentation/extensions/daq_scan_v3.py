...
from pymodaq_plugins_spectrum_instrumentation.extensions.custum_PID import *
...
N_SCANS_PER_PLAY = 5  # Number of scans acquired each time "play" is pressed
...
EXTENSION_NAME = 'DAQScan_v3'
CLASS_NAME = 'DAQScan_v3'


class DAQScan_v3(QObject, ParameterManager):
    """
    ...
    params = [
        ...
        {'title': 'Polarizer Scanning', 'name':'polarizer_scan', 'type': 'bool', 'value':True, 'children': [
            {'title': 'Start', 'name': 'start', 'type': 'float', 'value':0},
            {'title': 'End', 'name': 'end', 'type': 'float', 'value':90},
            {'title': 'Steps', 'name':'steps', 'type': 'int', 'value':4}
        ]}
    ]
    ...
    def __init__(self, dockarea: gutils.DockArea = None, dashboard: DashBoard = None):
        ...
        # Added For scaning angles
        self.angle_list = np.linspace(self.settings.child('polarizer_scan', 'start').value(), self.settings.child('polarizer_scan', 'end').value(), self.settings.child('polarizer_scan', 'steps').value())
        self._multi_scan_index = 0   # which scan we are on within a multi-scan play (0-based)
        self._multi_scan_running = False  # True while a multi-scan sequence is in progress
        self.description_backup = ""
    ...
    def create_dataset_settings(self):
        ...
        params_scan = [{'title': 'Scan information', 'name': 'scan_info', 'type': 'group', 'children': [
            ...
            {'title': 'Description:', 'name': 'description', 'type': 'text', 'value': ''},
            {'title': 'Scan Angle:', 'name': 'scan_angle', 'type': 'int', 'value': 0, 'readonly': True},
        ]}]
    ...
    def thread_status(self, status: utils.ThreadCommand):
        ...
        * "Timeout"
        ...
        if status.command == "Update_Status":
            ...
            # ---- Multi-scan logic ----
            if self._multi_scan_running:
                self._multi_scan_index += 1
                if self._multi_scan_index < len(self.angle_list):
                    # There are still scans to run — do NOT close the file yet, start next scan
                    logger.info(
                        f'Multi-scan: scan {self._multi_scan_index}/{N_SCANS_PER_PLAY} done, '
                        f'starting next one')
                    # Re-enable start briefly so _start_one_scan can proceed
                    self.ui.set_action_enabled('start', True)

                    # === Launch PID
                    customPID(self.modules_manager, self.angle_list[self._multi_scan_index])
                    self._start_one_scan()
                    return   # skip the normal "scan done" tear-down below
                else:
                    # All N scans finished
                    self._multi_scan_running = False
                    logger.info(f'Multi-scan: all {N_SCANS_PER_PLAY} scans completed')
            # ---- end multi-scan logic ----
    ...
    def set_metadata_about_current_scan(self):
        ...
        self.scan_attributes.child('scan_info', 'scan_angle').setValue(self.angle_list[self._multi_scan_index])

        # === Override description
        if self._multi_scan_running and self._multi_scan_index > 0:
            # If not first scan, then copy description from previous
            self.scan_attributes.child('scan_info', 'description').setValue( self.description_backup )
            return True

        if not self.batch_started:
            res = self.show_file_attributes('scan')

            self.description_backup = self.scan_attributes.child('scan_info', 'description').value()
        else:
            res = True
        return res
    ...
    def start_scan(self):
        """
        Entry point when the user presses "play".
        Launches a sequence of N_SCANS_PER_PLAY scans saved in the same dataset.
        """
        self._multi_scan_index = 0
        self._multi_scan_running = True
        self._start_one_scan()

    def _start_one_scan(self):
        """
        Internal helper: start the current scan in the multi-scan sequence.
        Replaces the original start_scan body for a single acquisition.
        """
        self.ui.display_status( f'Starting acquisition {self._multi_scan_index + 1}/{N_SCANS_PER_PLAY}')
        logger.info( f'Multi-scan: starting scan {self._multi_scan_index + 1}/{N_SCANS_PER_PLAY}')
    ...
    def stop_scan(self):
        ...
        self._multi_scan_running = False  # abort any ongoing multi-scan sequence
...
def main():
    ...
    if args.preset:
        preset_file_name = args.preset
    preset_file_name = "Chirals Full"
    # dashboard, extension, win = load_dashboard_with_preset(preset_file_name, 'DAQScan')
    dashboard, extension, win = load_dashboard_with_preset(preset_file_name, 'DAQScan_v3')
